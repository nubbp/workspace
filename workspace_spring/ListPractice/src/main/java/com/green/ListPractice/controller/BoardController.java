...
//    게시글 목록 페이지에서 제목 클릭 시 상세 페이지로 이동
    @GetMapping("/detail")
    public String regDetail(@RequestParam(name="boardNum") int boardNum, Model model) {
        System.out.println(boardList);
//        내가 클릭한 게시글의 상세 정보를 html에 뿌려주고 싶음
//        1. 내가 클릭한 게시글을 어떻게 구분하지?
//           => 글 번호로 구분
//        1) 내가 클릭한 게시글의 글번호를 알아야 함
//        2) 게시글 목록(boardList)에서 내가 가지고 온 글번호화 일치하는 게시글을 찾음
        BoardVO result = null;
        for(BoardVO board:boardList) {
            if(boardNum == board.getBoardNum()) {
                result = board;
            }
        }
