...
public class Bank {
    ...
    public Client matchAccNumName(String accNUmber, String name) {
        Client member = null;
        for (int i = 0; i < clientList.size(); i++) {
            if (accNUmber == clientList.get(i).getAccInfo().getAccNUmber() && name == clientList.get(i).getName()) {
                member = clientList.get(i);
            }
        }
        return member;
    }
    ...
    //메서드를 정의하시오.
    public List<Client> overBalanceList (int balance) {
        List<Client> overList = new ArrayList<>();
        for (int i = 0; i < clientList.size(); i++) {
            if (clientList.get(i).getAccInfo().getBalance() > balance) {
                overList.add(clientList.get(i));
            }
        }
        return overList;
    }
