...
-- 데이터의 변화가 생기는 경우 해당 변화가 잘못되지 않았는지 한 번 확인할 기회를 준다
-- 변화된 데이터가 잘못되지 않았으면 COMMIT; 명령어로 데이터 변화를 확정지음
-- 잘못된 변화가 있을 때 변화를 취소시키고 싶다면 ROLLBACK;
...
-- 컬럼명은 반드시 모든 컬럼이 작성될 필요가 없다
-- 작성되지 않은 컬럼에는 기본적으로 NULL이 들어간다
INSERT INTO test_member (MEM_NUM, MEM_NAME)
VALUES (12, '홍');

-- 아래 쿼리는 MEM_NUM에 NULL이 들어가는데
-- MEM_NUM은 기본키(PRIMARY KEY)라 NULL 값이 들어갈 수 없어 오류 발생
INSERT INTO test_member (MEM_NAME, MEM_AGE)
VALUES ('이' 30);

-- 테이블 생성 시 작성한 컬럼명 순서대로 데이터를 넣는다면 컬럼명 작성 생략 가능
-- 단, 모든 컬럼의 값이 들어가야 함
INSERT INTO test_member () VALUES (13, 'PARK', 22, '양산시');
...
-- 회원번호가 10번인 회원의 이름을 '김회원', 나이를 32살로 변경하는 쿼리 작성
UPDATE test_member
SET
	MEM_NAME = '김회원'
	, MEM_AGE = 32
...
-- 40살 이하인 회원을 모두 삭제
DELETE FROM test_member WHERE MEM_AGE <= 40;